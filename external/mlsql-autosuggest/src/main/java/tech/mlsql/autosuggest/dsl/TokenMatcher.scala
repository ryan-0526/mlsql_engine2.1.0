...
class TokenMatcher(tokens: List[Token], val start: Int) {
  ...
  private def matchToken(food: Food, currentIndex: Int): Int = {
    ...
    food.name match {
      case Some(name) =>
        println(tokens(currentIndex).getType)
        if (tokens(currentIndex).getType == food.tp && tokens(currentIndex).getText == name) {
        currentIndex
      } else -1
      case None =>
        println(tokens(currentIndex))
        println(tokens(currentIndex).getType)
    ...
    foods.foreach { foodw =>
      ...
      if (currentIndex <= -1 && !foodw.optional) {
        ...
        if (foodw.optional) {
          ...
          if (matchValue != -1) {
            currentIndex = currentIndex - stepSize    //避免出现不合理语句的表名也进行存储 例如 as dfs ;
          } else {
            //mark fail
            isFail = true
          }
