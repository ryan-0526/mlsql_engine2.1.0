...
object LexerUtils {
  ...
  def toTokenPos(tokens: List[Token], lineNum: Int, colNum: Int): TokenPos = {
    ...
    val oneLineTokens = tokens.zipWithIndex.filter { case (token, index) =>
      token.getLine == lineNum    //根据console传来的坐标信息，在所有token(所有sql拆分后的词)中找出用户输入需要提示的那一行语句
    }

    val firstToken = oneLineTokens.headOption match {
      case Some(head) => head   //获取语句头
      case None =>
    ...
    if (colNum < firstToken._1.getCharPositionInLine) {   //获取当前token，最后一个字符，所在行位置坐标
    ...
    if (colNum > lastToken._1.getCharPositionInLine
      ...
      return TokenPos(lastToken._2, TokenPosType.CURRENT, colNum - lastToken._1.getCharPositionInLine)    //返回当前语句位置，状态，偏移量值
    }
    oneLineTokens.map { case (token, index) =>
      ...
      //这个token是 [(,).]等，则不算              //colNum = end : 表示此时用户只输入一个字符
