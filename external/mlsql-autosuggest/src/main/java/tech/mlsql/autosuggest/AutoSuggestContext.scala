...
class AutoSuggestContext(val session: SparkSession,
  ...
  private var _rawTokens: List[Token] = List()  //存储所有语句中的词
  private var _statements = List[List[Token]]() //将同一行语句中的词存储一个集合
  ...
  def buildFromString(str: String): AutoSuggestContext = {
    build(lexer.tokenizeNonDefaultChannel(str).tokens.asScala.toList)   //将语句整理，去除空格，换行符，存入集合
    this
  }
  ...
  def suggest(lineNum: Int, columnNum: Int): List[SuggestItem] = {
    ...
    val tokenPos = LexerUtils.toTokenPos(rawTokens, lineNum, columnNum)   //返回当前要提示字符坐标，状态，偏移量值
    _suggest(tokenPos)
  }
  ...
  private[autosuggest] def _suggest(tokenPos: TokenPos): List[SuggestItem] = {
    ...
    val (relativeTokenPos, index) = toRelativePos(tokenPos) //返回当前用户输入字符所在_statement中对应sql所在位置中字符索引信息pos, sql在_statements中的索引位置
    ...
    val items = _statements(index).headOption.map(_.getText) match {    //匹配语句头关键字
      case Some("load") =>
      ...
      case Some("select") =>
        ...
        val relativeTokenPos = LexerUtils.toTokenPosForSparkSQL(LexerUtils.toRawSQLTokens(this, selectTokens), relativeLineNum, relativeColumnNum) //获取当前字符在list[token]中的索引
        if (isInDebugMode) {
